...
void MAP::MercatorProjCal( double longitude, double latitude, double *X, double *Y )
{
	...
	*Y = tan((90+latitude)*M_PI/360);
	*Y = log10(*Y)/log10(2.718282)/M_PI;
	printf("y==%f\n",Y);
	(*Y) *= EQUATOR_RADIUS;  
}
...
void MAP::context(double gps[3],GP_INFO* output)
{
	...
	if(locationInfo[3]!=-1)
	{
		...
		for(int i=locationInfo[2]+1;i<maxnum;i++)
		{
			...
			{
				founded=true;
				double x=current_line->points[i].x-gps[0],y=current_line->points[i].y-gps[1],yaw=-gps[2];
				output->scene.gls.gps[num].x=x*cos(yaw)-y*sin(yaw);
				output->scene.gls.gps[num].y=x*sin(yaw)+y*cos(yaw);
				num++;
			}
		}
