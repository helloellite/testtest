...
bool MAP::taskAtrributeComplete()
{
...
{
	//如果没有任务线，则生成之。
	if(task.points.size()!=0&&task.lineID.size()!=task.points.size()-1)
	{
...
bool MAP::locate(double x,double y,double yaw)
{
	if(!taskLocate(x,y,yaw))
		MBUG("Locate default");//return false;		//以后可以采用直接进行全局定位来做
	if(locationInfo[3]!=-1&&!localLocate(x,y,yaw))
		locationInfo[3]=-1;
	if(locationInfo[3]==-1&&!globleLocate(x,y,yaw))			//第一次局部定位失败，开启全局定位
		locationInfo[3]=-1;
	MBUG("locate int line %d at %d",locationInfo[3],locationInfo[2]);
...
{
	//在当前位置前方搜索若干点  若搜到结果，则直接返回
	if(locationInfo[3]==-1)	//不用navigator的信息
		return false;
	else	//任务点上未发生变化。 此时引导线ID未变——更新线内index即locationInfo[2]
	{//弱约束匹配：仅仅观察是否结束当前段，且下一段未结束。若是，则进入下一段（为了适应盲区：结束了当前段但是下一段未开始）。
		...
		while(li<locationInfo[2]+10*ind&&li<pcount-1&&ind==isInSegment(x,y,line->points[li].x,line->points[li].y,line->points[li+1].x,line->points[li+1].y))
			li+=ind;
		if(li<locationInfo[2]+10*ind&&li<pcount-1)		//找到了
		{	
			if(ind==1) li--;
			locationInfo[2]=li;
			return true;
...
bool MAP::loadTask(char* filename)
{
	if(task.points.size()!=0) 
	{
		...
		while(!file.eof())
		{
			...
			case 3:
				alt = 0.0;
				blh2xy(lat, lon, alt, point.x, point.y, z);
				break;
			case 4:
...
/*  以下代码利用了地图拓扑进行定位，处理速度快但是需地图拓扑。

bool MAP::taskLineComplete()	//根据navigator定位
{	
	int tnum=task.points.size();		//任务点个数
	int *index=new int[tnum];		//存放每个任务点所在的路口/道路下标。路口index=组内下标*2，road index=组内下标*2+1
	for(int i=0;i<tnum;i++)			
	{
		index[i]=-1;
		double x=task.points[i].x,y=task.points[i].y;
		int type=task.points[i].roadtype;
		int jidx=coor2Junction(x,y);
		switch (type/10)
		{
		case 0:case 3:case 4:
			{
				int ridx=coor2Road(x,y);	//仅计算所在道路，未精确到道线
				if(jidx==-1&&ridx==-1)
					MBUG("WARNING:task_point[%d] unmatched,work go on.Take care of the result and check the integrity of the map.\n ",i);
				else if(jidx!=-1&&ridx==-1)
					MBUG("WARNING: in-turned task_point[%d] located in junction.Take care of the result\n",i);
				else 	//ridx!=-1 优先分配
					index[i]=2*ridx+1;
				break;
			}
		case 2:		//前方缺少入点，补齐后方可继续
			MBUG("ERROR:Entrance point missing before task_point[%d],work break\n",i);
			return false;
		case 1:
			if(jidx==-1)
				MBUG("WARNING:No junction matched with task_point[%d].Take care of the result and check the integrity of the map.\n ",i);
			else
			{
				int exit_index=getExitPoint(i);
				if(exit_index==-1)
				{
					MBUG("ERROR:NO exit point founded matched with task_point[%d],work break\n",i);
					return false;
				}
				else/ *写这个else是为了防止以后把上个if语句中return改没了的时候 出错* /
				{
					index[i++]=jidx*2;
					while(i<=exit_index)
						if(isCoorInJunciton(task.points[i].x,task.points[i].y,jidx))	//出口点是否位于入口点的那个junction内
							index[i++]=jidx*2;
						else
						{
							MBUG("ERROR:point in turn_line error\n");	//拐弯的过程task[entrance]到task[exit]中，有点不在对应的路口内
							return false;
						}	
				}
			}
			break;
		default:
			MBUG("IMP WARNING:UNKNOW TASK ATTRIBUTE %d with POINT[%d]\n",type/10,i);
		}
	}

	//此时，每个任务点所属的道路或节点已经确定。
	//解析index数组，生成task.lineID
	//规则：对于定位到道路上的点，根据前后点的方向信息，确定道路行驶方向
	//		对于定位到junction上的点，根据多个点的信息，匹配
}*/
